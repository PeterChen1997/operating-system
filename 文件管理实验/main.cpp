...
void OpenMode(UFD *p,string name)
{
    ...
    for(i=0;i<5;i++)
    {
        if(f->point!=NULL&&f->point->filename==name){cout<<"THIS FILE IS ALREADY OPENED!"<<endl;return;}
        f=f->next;
    }
    for(i=0;i<5;i++)
    {
    ...
    if(i==5){cout<<"ERROR!YOU CAN'T OPEN THIS FILE,NUMBER MUST <=5!";}
    else
    {
        ...
        for(j=0;j<3;j++)
        {
            if(p->pro[j]!=protect[j]-48)break;
        }
        if(j!=3){cout<<"ERROR,OPEN MODE IS WRONG!"<<endl;return;}
        else
        {
...
void Create()
{
    ...
    if(n==10)
    {cout<<"CREAT FAILED!!THERE IS NO SPACE,FILE NUMBER MUST<=10!"<<endl;return;}
    else
    {
        q->filename=name;
        for(int i=0;i<3;i++)
...
void Open()
{
    ...
    if(n==10){cout<<"ERROR!THIS FILE IS NOT EXISTS!"<<endl;return;}
    else
    {
        OpenMode(q,name);
    }
...
void Close()
{
    ...
    if(n==10){cout<<"ERROR!CAN'T FIND THIS FILE!"<<endl;return;}
    else
    {
        AFD *f=openfile;
        int i;
        ...
        if(n==5){cout<<"ERROR!THIS FILE IS NOT OPENED!"<<endl;return;}\
		else
        {
            Close1(f);
        }
...
void Read()
{
    ...
    for(i=0;i<5;i++)
    {
        if(f->filenum==num&&f->point!=NULL)break;
        f=f->next;
    }
    if(i==5){cout<<"ERROR!THIS FILE IS NOT OPENED!"<<endl;return;}
    else
    {
        if(f->filepro[0]==1){cout<<"SUCCESS!READING!"<<endl;}
        else{cout<<"ERROR!THIS FILE CAN'T READ WITH READING RIGHT LIMITED!"<<endl;}
    }
    return;
}
void Write()
{
    ...
    else
    {
        ...
        else{cout<<"ERROR!THIS FILE CAN'T WRITE WITH WRITING RIGHT LIMITED!"<<endl;}
    }
    return;
}
void FileWork(int com)
{
    if(com==1){Create();}//执行create
    if(com==2){Delete();}//执行delete
    if(com==3){Open();} //执行open
    if(com==4){Close();}//执行close
    if(com==5){Read();}//执行read
    if(com==6){Write();}//执行write
    if(com==7)
    {
...
int main()
{
    ...
    while(x==0)
    {
        ...
        if(x==0)cout<<"YOUR NAME IS NOT IN THE USER NAME TABLE,TRY AGAIN."<<endl;
    }
    show();
    int com=0;
    while(com!=7)
